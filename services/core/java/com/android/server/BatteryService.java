...
public final class BatteryService extends SystemService {
    ...
    public void update(android.hardware.health.HealthInfo info) {
        ...
        synchronized (mLock) {
            if (!mUpdatesStopped) {
                mHealthInfo = info;
                // The vendor health HAL reports a bogus (battery-side, ~0) maxChargingCurrent,
                // so the lock screen shows 0.0W during real OnePlus fast charging. Substitute
                // the true input-side power before broadcasting.
                overrideOemChargingPowerLocked(mHealthInfo);
    ...
    // The vendor health HAL reports a battery-side (~0) maxChargingCurrent, so the lock
    // screen shows 0.0W during real OnePlus SuperVOOC/UFCS fast charging. When the oplus
    // charging driver reports arbitrated input power (cpa_power, milliwatts), substitute
    // true input-side values: voltage from the usb power_supply node, current = power/voltage.
    private static final File OEM_CPA_POWER_FILE =
            new File("/sys/class/oplus_chg/common/cpa_power");       // milliwatts
    private static final File OEM_USB_VOLTAGE_NOW_FILE =
            new File("/sys/class/power_supply/usb/voltage_now");     // microvolts

    private void overrideOemChargingPowerLocked(HealthInfo info) {
        if (!mHasOemCharger || info == null
                || info.batteryStatus != BatteryManager.BATTERY_STATUS_CHARGING) {
            return;
        }
        long powerMilliwatts = readOemSysfsLong(OEM_CPA_POWER_FILE);
        long inputMicrovolts = readOemSysfsLong(OEM_USB_VOLTAGE_NOW_FILE);
        if (powerMilliwatts <= 0 || inputMicrovolts <= 0) {
            return; // not fast charging, or nodes unreadable -> keep the HAL values
        }
        // current(uA) = power(W) / voltage(V), expressed in micro units:
        //   powerMilliwatts * 1e9 / inputMicrovolts
        long currentMicroamps = (powerMilliwatts * 1_000_000_000L) / inputMicrovolts;
        info.maxChargingVoltageMicrovolts = (int) inputMicrovolts;
        info.maxChargingCurrentMicroamps = (int) currentMicroamps;
    }

    private static long readOemSysfsLong(File file) {
        try {
            String s = FileUtils.readTextFile(file, 32, null).trim();
