...
public class QuickSettingsControllerImpl implements QuickSettingsController, Dumpable {
    ...
    private boolean isQsSplitShadeEnabledLegacy() {
    ...
    boolean isOpenQsEvent(MotionEvent event) {
        ...
        if (isQsSplitShadeEnabledLegacy() && mBarState == StatusBarState.SHADE) {
    ...
    public boolean getSplitShadeEnabledLegacy() {
        return isQsSplitShadeEnabledLegacy();
    }
    ...
    public boolean shouldQuickSettingsIntercept(float x, float y, float yDiff) {
        ...
        if (!isExpansionEnabled() || mCollapsedOnDown || (keyguardShowing
                ...
                || (isQsSplitShadeEnabledLegacy() && mBarState == StatusBarState.SHADE)) {
            return false;
        }
    ...
    private float getHeaderTranslation() {
        if (mSplitShadeEnabled || (isQsSplitShadeEnabledLegacy() && mBarState == StatusBarState.SHADE)) {
    ...
    private void updateQsSplitShadeState() {
        if (mQs != null) {
            mQs.setInSplitShade(mSplitShadeEnabled || (isQsSplitShadeEnabledLegacy() && mBarState == StatusBarState.SHADE));
        }
    }
    ...
    void updateExpansion() {
        ...
        final float adjustedExpansionFraction = (mSplitShadeEnabled || (isQsSplitShadeEnabledLegacy() && mBarState == StatusBarState.SHADE))
        ...
        if (isQsFragmentCreated()) {
            ...
            if (mEnableClipping) {
                mQs.setFancyClipping(
                        ...
                        clipBottom,
                        radius,
                        qsVisible && !mSplitShadeEnabled && !(isQsSplitShadeEnabledLegacy() && mBarState == StatusBarState.SHADE),
                        mIsFullWidth);
            }
    ...
    public float calculateNotificationsTopPadding(boolean isShadeExpanding,
        ...
        boolean keyguardShowing = mBarState == KEYGUARD;
        if (isQsSplitShadeEnabledLegacy() && mBarState == StatusBarState.SHADE) {
            return mQuickQsHeaderHeight + mResources.getDimensionPixelSize(R.dimen.notification_side_paddings) / 3;
        }
        if (mSplitShadeEnabled) {
    ...
    boolean handleTouch(MotionEvent event, boolean isFullyCollapsed,
        ...
        float statusBarThreshold = isQsSplitShadeEnabledLegacy() && mBarState == StatusBarState.SHADE
        ...
        if (action == MotionEvent.ACTION_DOWN && isFullyCollapsed && isExpansionEnabled()) {
            ...
            if (isQsSplitShadeEnabledLegacy()) {
        ...
        if (!isQsSplitShadeEnabledLegacy() && mTwoFingerExpandPossible && isOpenQsEvent(event) && isInStatusBar) {
