...
public class TouchAnimator {
    ...
    private static class FloatKeyframeSet<T> extends KeyframeSet {
        ...
        protected void interpolate(int index, float amount, Object target) {
            ...
            float val = firstFloat + (secondFloat - firstFloat) * amount;
            if (mProperty instanceof FloatProperty) {
                ((FloatProperty<T>) mProperty).setValue((T) target, val);
            } else {
                mProperty.set((T) target, val);
            }
        }
    }
    ...
    private static class IntKeyframeSet<T> extends KeyframeSet {
        ...
        protected void interpolate(int index, float amount, Object target) {
            ...
            int val = (int) (firstFloat + (secondFloat - firstFloat) * amount);
            if (mProperty instanceof IntProperty) {
                ((IntProperty<T>) mProperty).setValue((T) target, val);
            } else {
                mProperty.set((T) target, val);
            }
        }
