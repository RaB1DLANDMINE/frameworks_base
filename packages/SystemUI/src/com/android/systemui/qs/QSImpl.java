...
public class QSImpl implements QS, CommandQueue.Callbacks, StatusBarStateController.StateListener,
    ...
    public void setQsExpansion(float expansion, float panelExpansionFraction,
        ...
        boolean isAnimating = expansion > 0.0f && expansion < 1.0f;
        int layerType = isAnimating ? View.LAYER_TYPE_HARDWARE : View.LAYER_TYPE_NONE;

        if (mContainer != null && mContainer.getLayerType() != layerType) {
            mContainer.setLayerType(layerType, null);
        }
        if (mQSPanelController != null && mQSPanelController.getView() != null) {
            View qsView = mQSPanelController.getView();
            if (qsView.getLayerType() != layerType) {
                qsView.setLayerType(layerType, null);
            }
        }
    ...
    private float calculateAlphaProgress(float panelExpansionFraction) {
        boolean isLegacySplitShade = android.provider.Settings.System.getInt(
                getContext().getContentResolver(), Settings.System.QS_SPLIT_SHADE, 0) != 0 && getContext().getResources().getConfiguration().orientation != android.content.res.Configuration.ORIENTATION_LANDSCAPE;
